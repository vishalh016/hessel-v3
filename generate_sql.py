...
sql_lines = []
sql_lines.append("-- ============================================================")
sql_lines.append("-- HESSEL — Menu Items Seeding Script")
sql_lines.append("-- Run this in your Supabase SQL Editor")
sql_lines.append("-- ============================================================")
sql_lines.append("")
sql_lines.append("-- 1. Ensure the image_url column exists")
sql_lines.append("ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS image_url TEXT;")
sql_lines.append("")
sql_lines.append("-- 2. Clear existing menu items to prevent duplicates")
sql_lines.append("DELETE FROM menu_items;")
sql_lines.append("")
sql_lines.append("-- 3. Insert new menu items")
sql_lines.append("INSERT INTO menu_items (category_id, name, description, price_per_plate_addon, dietary_flags, is_active, display_order, image_url) VALUES")
...
for idx, item in enumerate(items):
    ...
    # Assign some realistic prices for premium items to make the pricing engine interesting!
    # Let's add addon price for some items like Mutton, Prawn, Fish, and Belgian Chocolate!
    price_addon = 0
    ...
    line = f"  ({category_subquery}, '{name}', NULL, {price_addon}, {dietary_flags}, true, {idx + 1}, {image_val})"
    if idx < len(items) - 1:
        line += ","
    else:
        line += ";"
...
with open('d:/Hessel-web/supabase/seed_menu_items.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(sql_lines))

print(f"Generated SQL seed file with {len(items)} items at supabase/seed_menu_items.sql")
